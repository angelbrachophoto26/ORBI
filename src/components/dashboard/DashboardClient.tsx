      ...
      <div className="flex items-start justify-between mb-6 md:mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-emerald-500 uppercase tracking-widest">Orbi</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Mis proyectos</h1>
          <p className="text-slate-400 text-sm mt-1">Gestiona tus estrategias de marketing</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {pendingInvites.length > 0 && (
            <div className="relative">
              ...
              {showInvites && (
                <div className="absolute right-0 top-10 w-80 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 p-3 space-y-2">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest px-1 mb-2">
                    Invitaciones pendientes
                  </p>
                  {pendingInvites.map((invite) => (
                    <div key={invite.id} className="flex items-center justify-between gap-3 p-3 bg-slate-800/60 rounded-lg">
                      <div className="min-w-0">
                        <p className="text-sm text-white font-medium truncate">{invite.projectName}</p>
                        <p className="text-xs text-slate-500">Te invitaron a colaborar</p>
                      </div>
                      <button
                        onClick={() => handleAcceptInvite(invite.id)}
                        className="shrink-0 flex items-center gap-1 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-medium rounded-lg transition-colors"
                      >
      ...
      {projects.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <button
            ...
            disabled={creating}
            className="flex flex-col items-center justify-center gap-3 h-48 border-2 border-dashed border-slate-800 rounded-xl text-slate-600 hover:border-emerald-600/50 hover:text-emerald-500 hover:bg-emerald-600/5 transition-all duration-200 cursor-pointer disabled:opacity-50"
          >
